...
var RATE_LIMIT_BASE_DELAY_MS = 5000;
...
export async function callGemini(options) {
  ...
  while (rateLimitAttempts < RATE_LIMIT_MAX_RETRIES) {
    ...
    var backoff = RATE_LIMIT_BASE_DELAY_MS * Math.pow(2, rateLimitAttempts);
    ...
    if (remaining < waitMs + 3000) {
      console.error('[' + label + '] RATE LIMITED (429) by Gemini and out of time budget: ' +
        'needed ' + waitMs + 'ms of backoff but only ' + Math.max(0, remaining) + 'ms remain. ' +
        'Giving up after ' + rateLimitAttempts + ' retries.');
      break;
    }
