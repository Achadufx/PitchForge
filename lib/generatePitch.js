...
// True when haystack contains at least minWords consecutive words from needle.
// Quotes are matched this way because a model may legitimately cite a fragment
// of a long quote rather than the whole sentence, and that still counts as
// using the real quote instead of inventing one.
function containsWordRun(haystack, needle, minWords) {
  var hay = normalizeForMatch(haystack);
  var words = normalizeForMatch(needle).split(' ').filter(Boolean);
  if (!words.length || !hay) return false;
  if (words.length <= minWords) return hay.indexOf(words.join(' ')) !== -1;
  for (var i = 0; i + minWords <= words.length; i++) {
    if (hay.indexOf(words.slice(i, i + minWords).join(' ')) !== -1) return true;
  }
  return false;
}
...
// True when the opening cites a hard, checkable fact — the verbatim quote, the
// fund name, or a named company. A thesis restatement deliberately does NOT
// count: that is precisely the portable, could-be-anyone opening the test
// exists to catch.
function openingUsesAnchor(body, anchors) {
  ...
  for (var i = 0; i < anchors.length; i++) {
    ...
    if (!anchor || !anchor.value) continue;
    if (anchor.type === 'quote') {
      if (containsWordRun(opening, anchor.value, 5)) return true;
    } else {
      var name = normalizeForMatch(anchor.value);
...
export async function generatePitch(investorName, firm, startupName, description, ask, research) {
  ...
  // Hard, checkable facts the opening line is allowed to cite: the verbatim
  // quote, the fund name, named companies. Thesis text is deliberately excluded.
  var anchors = [];
  ...
  if (research) {
    ...
    // The quote goes FIRST and is fenced off, because it is the one field that
    // must be reproduced character-for-character. Everything else can be
    // paraphrased; a misquote is attributing invented words to a real person.
    if (research.publicQuote) {
      researchBlock += '- THEIR EXACT WORDS' +
        (research.quoteSource ? ' (' + research.quoteSource + ')' : '') + ', verified: "' +
        research.publicQuote + '"\n' +
        '  Reproduce this EXACTLY as written above, inside quotation marks. Do not reword it, ' +
        'do not shorten it into a paraphrase, do not modernise the phrasing, and do not attribute ' +
        'any other sentence to them. If you cannot fit it verbatim, use a named deal instead.\n';
      citableFacts.push('quote');
      anchors.push({ type: 'quote', value: research.publicQuote });
    } else {
      researchBlock += '- NO verified quote exists for this investor. You therefore may NOT put any ' +
        'sentence in quotation marks and attribute it to them. Open on a named deal or the fund ' +
        'name instead.\n';
    }
    ...
    // Named fund and companies are the non-quote anchors the opening may use.
    // Deals/portfolio entries arrive as "Company (year, round)", so the bare
    // name is what a sentence would actually contain.
  ...
  for (var ai = 0; ai < anchors.length; ai++) {
    if (anchors[ai].type !== 'quote') anchorNames.push(anchors[ai].value);
  }

  var hasQuote = !!(research && research.publicQuote) && !lowConfidence;
  ...
  if (lowConfidence || !anchors.length) {
    ...
    openingRule = '- Line 1 must contain ONE of these, exactly as written in the research above:\n' +
      (hasQuote
        ? '    (a) their exact quote, reproduced verbatim inside quotation marks — preferred, or\n'
        : '') +
      '    (' + (hasQuote ? 'b' : 'a') + ') a named fund' +
      (firm ? ' — for this investor that is "' + firm + '"' : '') + ', or\n' +
      '    (' + (hasQuote ? 'c' : 'b') + ') a named portfolio company or deal' +
      (anchorNames.length ? ' — e.g. ' + anchorNames.slice(0, 4).join(', ') : '') + '.\n' +
      '  A restatement of their thesis does NOT satisfy this. Neither does "your focus on African ' +
      'fintech" or any sector description. It must be a proper noun or their actual words.\n';
  }
  ...
  var prompt = 'You write cold emails that get replies from investors. Founders forward yours as examples of how it should be done.\n\n' +
    ...
    'The opening line must contain a proper noun or their actual words: the exact quote in quotation ' +
    'marks, a named fund, or a named portfolio company. A description of their thesis or sector focus ' +
    'FAILS, however well written, because it could be sent to fifty other investors unchanged.\n' +
    'Before finishing, reread line 1. If it contains no quotation marks and no capitalised name that ' +
    'appears in the research above, rewrite it.\n\n' +
    'VOICE:\n' +
    ...
    'Never place a sentence in quotation marks and attribute it to this investor unless it is the ' +
    'exact verified quote given above.\n\n' +
  ...
  for (var attempt = 0; attempt < TOKEN_BUDGETS.length && callsMade < MAX_CALLS; attempt++) {
    ...
    if (result.ok) {
      ...
      if (candidate && qualityRetries < MAX_QUALITY_RETRIES) {
        ...
        // A quoted sentence that is not the verified quote is a fabricated
        // attribution — the single worst failure mode in a cold email.
        var quotedSpans = candidate.body.match(/["“]([^"”]{12,})["”]/g) || [];
        if (quotedSpans.length) {
          for (var qi = 0; qi < quotedSpans.length; qi++) {
            var inner = quotedSpans[qi].replace(/^["“]/, '').replace(/["”]$/, '');
            var isRealQuote = hasQuote && containsWordRun(research.publicQuote, inner, 5);
            // Quoting our own product language or a startup fact back is fine.
            var isOurs = containsWordRun(description, inner, 4) ||
              containsWordRun(startupName, inner, 2) ||
              (ask && containsWordRun(ask, inner, 3));
            if (!isRealQuote && !isOurs) {
              complaint += hasQuote
                ? 'You put words in quotation marks that are NOT their verified quote. The ONLY ' +
                  'sentence you may attribute to them is exactly this: "' + research.publicQuote +
                  '". Remove the invented quote.\n'
                : 'You attributed a quote to this investor, but NO verified quote exists for them. ' +
                  'Remove all quotation marks around anything attributed to them and open on a ' +
                  'named deal or the fund name instead.\n';
              break;
            }
          }
        }
        ...
        if (!openingUsesAnchor(candidate.body, anchors)) {
          complaint += 'Your opening line fails the portability test: it contains no verified ' +
            'proper noun and no verbatim quote, so it could be sent to any other investor ' +
            'unchanged. Rewrite line 1 so it contains ' +
            (hasQuote ? 'their exact quote in quotation marks, or ' : '') +
            'a named fund or portfolio company from the research' +
  ...
  console.log('generatePitch: done for ' + investorName +
    ...
    ' | quote available: ' + (hasQuote ? 'yes' : 'no') +
