...
export async function generatePitch(investorName, firm, startupName, description, ask, research) {
  ...
  // A 150-200 word email is ~350 output tokens. The budget is far above that
  // because it is shared with the model's thinking tokens on reasoning models;
  // callGemini disables thinking where supported, and this leaves headroom where
  // it cannot. Too low a value truncated the body mid-sentence.
  var TOKEN_BUDGETS = [3072, 6144];

  var result = null;
  for (var attempt = 0; attempt < TOKEN_BUDGETS.length; attempt++) {
    try {
      result = await callGemini({
        ...
        timeoutMs: 12000,
        // Shares the 60s function budget with researchInvestor (13s) and the 12s
        // inter-call delay: 13 + 12 + 14 + 12 = 51s worst case.
        //
        // Both values sit below the 15s first backoff step + 3s reserve, so the
        // server never begins a sleep it will be killed partway through. On a
        // 429 it returns immediately and pages/app.js performs the real
        // 15s/30s/60s wait, where there is no execution limit.
        deadlineMs: attempt === 0 ? 14000 : 12000,
        label: 'generatePitch'
      });
    } catch (err) {
    ...
    if (result.truncated && attempt < TOKEN_BUDGETS.length - 1) {
      console.warn('generatePitch: output truncated for ' + investorName + ' at ' +
        TOKEN_BUDGETS[attempt] + ' tokens; retrying at ' + TOKEN_BUDGETS[attempt + 1] + '.');
      continue;
    }
    break;
  }
  ...
  if (!result || !result.ok) {
    if (result && result.rateLimited) {
      ...
      var rateErr = new Error('Gemini rate limit exceeded. Wait a minute and retry this investor.');
      rateErr.rateLimited = true;
      throw rateErr;
    }
    console.error('generatePitch: Gemini call failed for ' + investorName + ' — ' +
