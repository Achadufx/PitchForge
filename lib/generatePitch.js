  ...
  for (var attempt = 0; attempt < TOKEN_BUDGETS.length; attempt++) {
    try {
      result = await callGemini({
        ...
        timeoutMs: 18000,
        // Shares the 60s function budget with researchInvestor's 20s. Pitch
        // generation gets the larger share since it can also retry on truncation.
        deadlineMs: attempt === 0 ? 22000 : 14000,
        label: 'generatePitch'
      });
    } catch (err) {
