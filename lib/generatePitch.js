export async function generatePitch(investorName, firm, startupName, description, ask, research) {
  if (!process.env.AGENTROUTER_API_KEY) {
    console.error('AGENTROUTER_API_KEY not set');
    return null;
  }
  ...
  var prompt = 'Write a cold pitch email to ' + investorName + (firm ? ' at ' + firm : '') + ' about ' + startupName + '.\n\n' +
    ...
    '- Line 4-5: What we built + one honest proof point\n' +
  ...
  try {
    ...
    if (data.error) {
      console.error('AgentRouter pitch error:', data.error);
      return null;
    }
    ...
    console.log('Pitch generated for:', investorName, '| Research used:', !!research);
