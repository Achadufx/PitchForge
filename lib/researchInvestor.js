...
// Normalizes to an array of non-empty strings. The model is told to use null for
// unknown fields, and pages/api/research-and-pitch.js calls .some() on these —
...
export async function researchInvestor(investorName, firm) {
  ...
  //
  // The anti-fabrication rules are deliberately blunt. This model has no web
  // access and a fixed training cutoff, so for a lesser-known investor the honest
  // answer is usually "unknown" — and a plausible invention here ends up pasted
  // into a real email to that person. Empty fields are recoverable; wrong ones
  // are not. generatePitch drops to its no-facts opening when nothing is citable.
  var prompt = 'You are a venture capital research analyst. Profile this investor: ' + who + '.\n\n' +
    ...
    '  "isRealPerson": true\n' +
    '}\n\n' +
    'HARD RULES — read these before answering:\n' +
    '- You have no web access. Report ONLY what you already know with confidence.\n' +
    '- Never invent a deal, quote, number, or date. A fabricated fact is far worse than a\n' +
    '  missing one, because it will be pasted into a real email to this person and they\n' +
    '  will know immediately that it is wrong.\n' +
    '- Use null for any unknown string field and [] for any unknown array. Do not guess,\n' +
    '  do not infer from the name, and do not fill fields just to look complete.\n' +
    '- publicQuote must be a real verbatim quote or null. Never paraphrase into that field.\n' +
    '- If you do not clearly recognize this specific person, set isRealPerson to false,\n' +
    '  confidence to "low", and leave the factual fields null/[]. That is the correct\n' +
    '  answer for most investors, and returning it is a success, not a failure.\n' +
  ...
  try {
    result = await callGroq({
      ...
      timeoutMs: 12000,
      jsonMode: true,
      // Caps request + rate-limit backoff. research-and-pitch runs this, a 2s
      // inter-call delay, and the pitch call inside one 60s Vercel invocation:
      // 13 + 2 + 20 + 14 = 49s worst case.
      deadlineMs: 13000,
      label: 'researchInvestor'
    });
  } catch (err) {
    console.error('researchInvestor: unexpected error calling Groq for ' + who + ': ' +
  ...
  if (!result.ok) {
    ...
    console.error('researchInvestor: Groq call failed for ' + who + ' — ' + result.error);
    return null;
  }
  ...
  try {
    ...
    if (!research.isRealPerson) {
      console.warn('researchInvestor: model could not confidently identify "' + who +
