...
export async function researchInvestor(investorName, firm) {
  ...
  //
  // The anti-fabrication rules are deliberately blunt. The model works from
  // training data with a fixed cutoff, so for a lesser-known investor "unknown"
  // is often the correct answer — and a plausible invention here ends up pasted
  ...
  var prompt = 'You are a venture capital research analyst. Profile this investor: ' + who + '.\n\n' +
    ...
    '- Report ONLY what you actually know from training. You cannot browse, so anything\n' +
    '  after your cutoff is unknown by definition — say so rather than extrapolating.\n' +
  ...
  try {
    ...
    result = await callGemini({
      ...
      // inter-call delay, and the Groq pitch call inside one 60s Vercel
      // invocation: 13 + 2 + 20 + 14 = 49s worst case.
      //
