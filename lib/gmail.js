...
/**
 * Gmail integration — OAuth, token custody, and reply detection.
 *
 * WHY THIS EXISTS
 * Resend can tell us a pitch was delivered and opened, because both events
 * happen on Resend's side of the wire. It cannot tell us about a reply: the
 * investor hits Reply and the message goes to the founder's own inbox, never
 * touching Resend. That is why reply rate reads 0% for founders who are
 * genuinely getting answers. Reading the founder's mailbox is the only place
 * that signal exists.
 *
 ...
 *
 * TOKEN CUSTODY
 * A Google refresh token is a standing key to somebody's mailbox. Three rules
...
// Read-only is the whole grant. PitchWire never sends, deletes, or modifies
// mail, and asking for more would both fail Google's verification review and
// make the consent screen say something untrue about what we do.
...
async function gmailGet(path, accessToken) {
  ...
  if (res.status === 403) {
    ...
    if (body.includes('insufficient') || body.includes('ACCESS_TOKEN_SCOPE')) {
      throw new GmailAuthError('The Gmail grant is missing the read scope');
    }
    throw new GmailError('Gmail rate limited this request');
  }
  if (!res.ok) {
...
export async function syncConnection(db, connection, options) {
  ...
  // Refresh ahead of expiry rather than reacting to a 401 halfway through.
  ...
  const { data: pending, error: pendingError } = await db
    .from('sent_emails')
    .select('id, to_email, sent_at, replied_at, relationship_id')
  ...
  for (const id of unique) {
    ...
    const target = candidates.find((row) => row.sent_at <= receivedAt && !stamped.has(row.id));
    if (!target) continue;
