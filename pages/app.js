    ...
    <div style={{ width: 220, background: "#080808", borderRight: "1px solid rgba(255,255,255,0.07)", display: "flex", flexDirection: "column", height: "100vh", position: "fixed", left: 0, top: 0, zIndex: 50 }}>
      ...
      <div style={{ padding: "16px", borderTop: "1px solid rgba(255,255,255,0.07)" }}>
        <div style={{ background: "#0f0f0f", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 10, padding: "14px" }}>
          ...
          {plan === "free" && (
            <button onClick={() => setActiveTab("account")} style={{ width: "100%", padding: "8px", borderRadius: 7, fontSize: 12, fontWeight: 700, cursor: "pointer", background: "#7c3aed", color: "#fff", border: "none" }}>
              Upgrade →
            </button>
          )}
          {plan === "starter" && (
            <button onClick={() => setActiveTab("account")} style={{ width: "100%", padding: "8px", borderRadius: 7, fontSize: 12, fontWeight: 700, cursor: "pointer", background: "rgba(124,58,237,0.15)", color: "#a78bfa", border: "1px solid rgba(124,58,237,0.2)" }}>
              Upgrade to Pro →
            </button>
          )}
          {plan === "pro" && (
            <div style={{ fontSize: 11, color: "#4ade80", fontWeight: 600, textAlign: "center" }}>✓ Pro — Full access</div>
          )}
        </div>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8, marginTop: 12, padding: "8px 4px" }}>
          ...
          <button onClick={onSignOut} title="Sign out" style={{ background: "none", border: "none", cursor: "pointer", color: "rgba(255,255,255,0.2)", fontSize: 14, padding: 4, transition: "color 0.2s" }}>→</button>
        </div>
      </div>
    </div>
  );
        ...
        return (
          <div key={i} style={{ display: "flex", alignItems: "center", flex: i < labels.length - 1 ? 1 : "none" }}>
            <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 4 }}>
              <div style={{ width: 28, height: 28, borderRadius: "50%", background: done ? "#10b981" : active ? "#7c3aed" : "#1e293b", border: "2px solid " + (done ? "#10b981" : active ? "#7c3aed" : "#334155"), color: done || active ? "#fff" : "#64748b", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 11, fontWeight: 700 }}>
                {done ? "✓" : i + 1}
              </div>
              <span style={{ fontSize: 10, fontWeight: active ? 700 : 400, color: active ? "#a78bfa" : done ? "#10b981" : "#475569", whiteSpace: "nowrap" }}>{label}</span>
            </div>
            {i < labels.length - 1 && <div style={{ flex: 1, height: 2, background: done ? "#10b981" : "#1e293b", margin: "0 6px", marginBottom: 16 }} />}
          </div>
        );
      })}
    </div>
        ...
        <div style={{ background: "rgba(124,58,237,0.08)", border: "1px solid rgba(124,58,237,0.25)", borderRadius: 14, padding: 32, textAlign: "center" }}>
          ...
          <div style={{ display: "flex", gap: 10, justifyContent: "center", flexWrap: "wrap" }}>
            ...
            <a href="/upgrade" style={{ background: "#7c3aed", color: "#fff", padding: "11px 24px", borderRadius: 8, fontWeight: 700, fontSize: 14, textDecoration: "none" }}>
              {plan === "starter" ? "Upgrade to Pro — $79/mo →" : "Upgrade to Pro — $79/mo →"}
            </a>
          </div>
        </div>
      ) : (
...
function UploadStep({ onNext }) {
  ...
  return (
    <div>
      ...
      <p style={{ color: "#64748b", marginBottom: 20, fontSize: 13 }}>CSV needs: name, email, firm (optional)</p>
      <div onDragOver={(e) => { e.preventDefault(); setDragging(true); }} onDragLeave={() => setDragging(false)} onDrop={(e) => { e.preventDefault(); setDragging(false); handleFile(e.dataTransfer.files[0]); }} onClick={() => document.getElementById("csv-input").click()} style={{ border: "2px dashed " + (dragging ? "#7c3aed" : "#334155"), borderRadius: 12, padding: "40px 24px", textAlign: "center", cursor: "pointer", background: dragging ? "rgba(124,58,237,0.05)" : "#0a0f1e", transition: "all 0.15s" }}>
      ...
      {investors.length > 0 && (
        <div style={{ marginTop: 20 }}>
          ...
          <button onClick={() => onNext(investors)} style={{ background: "#7c3aed", color: "#fff", border: "none", borderRadius: 8, padding: "12px 28px", fontWeight: 700, fontSize: 14, cursor: "pointer", width: "100%" }}>
...
function DescribeStep({ onNext, onBack }) {
  ...
  return (
    <div>
      ...
      <p style={{ color: "#64748b", marginBottom: 20, fontSize: 13 }}>The more specific you are, the better the AI pitches.</p>
      {[
...
function SendStep({ pitches, onRestart }) {
  ...
  return (
    <div>
      ...
      <p style={{ color: "#64748b", marginBottom: 20, fontSize: 13 }}>Your name will appear as the sender to investors.</p>
  ...
  return (
    <div>
      <div style={{ marginBottom: 28 }}>
        ...
        <p style={{ fontSize: 13, color: "rgba(255,255,255,0.35)" }}>Find investors that match your startup — filtered by stage, sector, and geography.</p>
      </div>
      <div style={{ background: "#0f0f0f", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 16, padding: "48px 32px", textAlign: "center" }}>
        ...
        <p style={{ fontSize: 14, color: "rgba(255,255,255,0.35)", maxWidth: 380, margin: "0 auto", lineHeight: 1.6 }}>
          We're building a database of thousands of investors with fit scoring, activity tracking, and deep AI research. For now, upload your investor list as a CSV on the Campaign tab.
        </p>
        {plan === "free" && (
          <a href="/upgrade" style={{ display: "inline-block", marginTop: 24, background: "#7c3aed", color: "#fff", padding: "10px 24px", borderRadius: 8, fontWeight: 700, fontSize: 13, textDecoration: "none" }}>
  ...
  return (
    <div>
      ...
      <div style={{ background: "#0f0f0f", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 14, padding: 24, marginBottom: 16 }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
          <div>
            ...
            <div style={{ fontSize: 13, color: "rgba(255,255,255,0.35)" }}>{pitchCount} of {limit} pitches used this month</div>
          </div>
          <div style={{ fontSize: 11, fontWeight: 700, padding: "4px 12px", borderRadius: 99, background: plan === "pro" ? "rgba(74,222,128,0.1)" : plan === "starter" ? "rgba(124,58,237,0.15)" : "rgba(255,255,255,0.06)", color: plan === "pro" ? "#4ade80" : plan === "starter" ? "#a78bfa" : "rgba(255,255,255,0.3)", textTransform: "uppercase", letterSpacing: "0.5px" }}>
            {plan}
          </div>
        </div>
      ...
      <div style={{ background: "#0f0f0f", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 14, padding: 24 }}>
        ...
        <button onClick={onSignOut} style={{ background: "transparent", color: "#f87171", border: "1px solid rgba(248,113,113,0.2)", borderRadius: 8, padding: "10px 20px", fontWeight: 600, fontSize: 13, cursor: "pointer" }}>
          Sign out
        </button>
      </div>
    </div>
  ...
  return (
    <>
      ...
      <div style={{ display: "flex", minHeight: "100vh", background: "#000", fontFamily: "'Inter', system-ui, sans-serif" }}>
        ...
        <main style={{ marginLeft: 220, flex: 1, padding: "40px 40px", overflowY: "auto", minHeight: "100vh" }}>
