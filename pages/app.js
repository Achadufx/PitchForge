      ...
      <div style={{ padding: "16px", borderTop: "1px solid rgba(255,255,255,0.07)" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8, marginTop: 12, padding: "8px 4px" }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ fontSize: 12, fontWeight: 600, color: "#fff", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{user?.user_metadata?.full_name || "Founder"}</div>
        ...
        return (
          <div key={i} style={{ display: "flex", alignItems: "center", flex: i < labels.length - 1 ? 1 : "none" }}>
            <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 4 }}>
              ...
              <span style={{ fontSize: 9, fontWeight: active ? 700 : 400, color: active ? "#a78bfa" : done ? "#10b981" : "#475569", whiteSpace: "nowrap" }}>{label}</span>
            </div>
...
function DocumentUpload({ onComplete, plan }) {
  ...
  const handleFiles = async (files) => {
    ...
    if (docs.length + arr.length > docLimit) {
      setError("You can upload up to " + docLimit + " documents on your current plan.");
      return;
    }
    const newDocs = [];
    for (const file of arr) {
      ...
      newDocs.push({ name: file.name, base64, mimeType: file.type, status: "pending", text: "" });
    }
    ...
    try {
      ...
      for (let i = 0; i < docs.length; i++) {
        ...
        const res = await fetch("/api/extract-text", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ base64: docs[i].base64, mimeType: docs[i].mimeType, name: docs[i].name }),
        });
  ...
  return (
    <div>
      <div style={{ marginBottom: 20 }}>
        <h3 style={{ fontSize: 15, fontWeight: 700, color: "#f1f5f9", marginBottom: 4 }}>Upload your startup documents</h3>
        <p style={{ fontSize: 12, color: "#64748b" }}>
          PDF, DOCX, or TXT — pitch deck, whitepaper, executive summary, business plan, anything.
          {plan === "free" && <span style={{ color: "#f87171" }}> Free plan: up to 3 documents.</span>}
        </p>
      </div>
      ...
      <div
        ...
        style={{ border: "2px dashed #334155", borderRadius: 10, padding: "28px 20px", textAlign: "center", cursor: "pointer", background: "#0a0f1e", transition: "all 0.15s", marginBottom: 12 }}
      >
        <div style={{ fontSize: 28, marginBottom: 8 }}>📄</div>
        <div style={{ fontWeight: 600, color: "#e2e8f0", fontSize: 13, marginBottom: 4 }}>Drop files here or click to browse</div>
        <div style={{ fontSize: 11, color: "#475569" }}>PDF · DOCX · TXT · Up to {docLimit} files</div>
        <input id="doc-input" type="file" multiple accept=".pdf,.docx,.txt" style={{ display: "none" }} onChange={(e) => handleFiles(e.target.files)} />
      </div>
      ...
      {docs.length > 0 && (
        <div style={{ display: "flex", flexDirection: "column", gap: 8, marginBottom: 16 }}>
          {docs.map((doc, i) => (
            <div key={i} style={{ display: "flex", alignItems: "center", gap: 10, background: "#0f172a", border: "1px solid #1e293b", borderRadius: 8, padding: "10px 14px" }}>
              <span style={{ fontSize: 16 }}>{doc.name.endsWith(".pdf") ? "📕" : doc.name.endsWith(".docx") ? "📘" : "📄"}</span>
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontSize: 12, fontWeight: 600, color: "#e2e8f0", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{doc.name}</div>
                <div style={{ fontSize: 10, color: doc.status === "done" ? "#4ade80" : doc.status === "extracting" ? "#a78bfa" : "#475569" }}>
                  {doc.status === "done" ? "✓ Extracted" : doc.status === "extracting" ? "⏳ Extracting..." : "Ready"}
                </div>
              </div>
              {!analyzing && <button onClick={() => removeDoc(i)} style={{ background: "none", border: "none", color: "#475569", cursor: "pointer", fontSize: 16, padding: 2 }}>×</button>}
            </div>
          ))}
        </div>
      )}
      ...
      {docs.length > 0 && (
        <button onClick={extractAndAnalyze} disabled={analyzing} style={{ width: "100%", padding: "11px", borderRadius: 8, fontSize: 14, fontWeight: 700, cursor: analyzing ? "not-allowed" : "pointer", background: analyzing ? "#1e293b" : "#7c3aed", color: analyzing ? "#475569" : "#fff", border: "none" }}>
          {analyzing ? "⏳ Analyzing your documents..." : "Analyze Documents with AI →"}
        </button>
      )}
    </div>
  );
...
function DescribeStep({ onNext, onBack, plan }) {
  ...
  if (mode === "review" && profile) {
    return (
      <div>
        <h2 style={{ fontSize: 18, fontWeight: 700, marginBottom: 4, color: "#f1f5f9" }}>AI understood your startup</h2>
        <p style={{ color: "#64748b", marginBottom: 16, fontSize: 13 }}>Review and edit before generating pitches.</p>

        <div style={{ background: "#0a0f1e", border: "1px solid rgba(124,58,237,0.2)", borderRadius: 10, padding: 16, marginBottom: 16 }}>
          <div style={{ fontSize: 10, fontWeight: 700, color: "#a78bfa", textTransform: "uppercase", letterSpacing: "1px", marginBottom: 10 }}>AI Analysis</div>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
            {[
              ...
              <div key={i}>
                <div style={{ fontSize: 9, fontWeight: 700, color: "rgba(255,255,255,0.25)", textTransform: "uppercase", letterSpacing: "0.5px", marginBottom: 2 }}>{k}</div>
        ...
        {[
          ...
          <div key={key} style={{ marginBottom: 14 }}>
            <label style={{ display: "block", fontWeight: 600, fontSize: 11, color: "#94a3b8", marginBottom: 5, textTransform: "uppercase", letterSpacing: "0.5px" }}>{label}</label>
            {multiline
              ? <textarea value={startup[key]} onChange={(e) => setStartup({ ...startup, [key]: e.target.value })} rows={4} style={{ width: "100%", borderRadius: 8, border: "1px solid #1e293b", padding: "10px 12px", fontSize: 13, resize: "vertical", outline: "none", fontFamily: "inherit", boxSizing: "border-box", background: "#0a0f1e", color: "#e2e8f0" }} />
              : <input value={startup[key]} onChange={(e) => setStartup({ ...startup, [key]: e.target.value })} style={{ width: "100%", borderRadius: 8, border: "1px solid #1e293b", padding: "10px 12px", fontSize: 13, outline: "none", boxSizing: "border-box", background: "#0a0f1e", color: "#e2e8f0" }} />
            }
          </div>
        ))}

        <div style={{ display: "flex", gap: 10 }}>
          <button onClick={() => setMode("upload")} style={{ background: "#1e293b", color: "#94a3b8", border: "none", borderRadius: 8, padding: "11px 18px", fontWeight: 600, fontSize: 13, cursor: "pointer" }}>← Re-upload</button>
          <button onClick={onBack} style={{ background: "#1e293b", color: "#94a3b8", border: "none", borderRadius: 8, padding: "11px 18px", fontWeight: 600, fontSize: 13, cursor: "pointer" }}>← Back</button>
          <button onClick={() => onNext(startup)} disabled={!valid} style={{ flex: 1, background: valid ? "#7c3aed" : "#1e293b", color: valid ? "#fff" : "#475569", border: "none", borderRadius: 8, padding: "11px 20px", fontWeight: 700, fontSize: 14, cursor: valid ? "pointer" : "not-allowed" }}>
            Generate Pitches ⚡
          </button>
        </div>
      </div>
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", gap: 0, background: "#0a0f1e", border: "1px solid #1e293b", borderRadius: 8, padding: 4, marginBottom: 20 }}>
        <button onClick={() => setMode("upload")} style={{ flex: 1, padding: "8px", borderRadius: 6, border: "none", cursor: "pointer", background: mode === "upload" ? "#7c3aed" : "transparent", color: mode === "upload" ? "#fff" : "#64748b", fontFamily: "inherit", fontSize: 13, fontWeight: 600, transition: "all 0.15s" }}>
          📄 Upload Documents
        </button>
        <button onClick={() => setMode("manual")} style={{ flex: 1, padding: "8px", borderRadius: 6, border: "none", cursor: "pointer", background: mode === "manual" ? "#7c3aed" : "transparent", color: mode === "manual" ? "#fff" : "#64748b", fontFamily: "inherit", fontSize: 13, fontWeight: 600, transition: "all 0.15s" }}>
          ✏️ Type Manually
        </button>
      </div>

      {mode === "upload" && <DocumentUpload onComplete={handleProfileComplete} plan={plan} />}

      {mode === "manual" && (
        <div>
          {[
            { key: "name", label: "Startup name", placeholder: "e.g. ForcepX" },
            { key: "description", label: "What you do", placeholder: "e.g. We give patients cryptographic control over their medical records. Every access is logged in a tamper-proof audit trail.", multiline: true },
            { key: "ask", label: "What you're raising", placeholder: "e.g. $500K pre-seed" },
          ].map(({ key, label, placeholder, multiline }) => (
            <div key={key} style={{ marginBottom: 16 }}>
              <label style={{ display: "block", fontWeight: 600, fontSize: 11, color: "#94a3b8", marginBottom: 6, textTransform: "uppercase", letterSpacing: "0.5px" }}>{label}</label>
              {multiline
                ? <textarea value={startup[key]} onChange={(e) => setStartup({ ...startup, [key]: e.target.value })} placeholder={placeholder} rows={3} style={{ width: "100%", borderRadius: 8, border: "1px solid #1e293b", padding: "10px 12px", fontSize: 13, resize: "vertical", outline: "none", fontFamily: "inherit", boxSizing: "border-box", background: "#0a0f1e", color: "#e2e8f0" }} />
                : <input value={startup[key]} onChange={(e) => setStartup({ ...startup, [key]: e.target.value })} placeholder={placeholder} style={{ width: "100%", borderRadius: 8, border: "1px solid #1e293b", padding: "10px 12px", fontSize: 13, outline: "none", boxSizing: "border-box", background: "#0a0f1e", color: "#e2e8f0" }} />
              }
            </div>
          ))}
          <div style={{ display: "flex", gap: 10 }}>
            ...
            <button onClick={() => onNext(startup)} disabled={!valid} style={{ flex: 1, background: valid ? "#7c3aed" : "#1e293b", color: valid ? "#fff" : "#475569", border: "none", borderRadius: 8, padding: "12px 28px", fontWeight: 700, fontSize: 14, cursor: valid ? "pointer" : "not-allowed" }}>
              Generate Pitches ⚡
            </button>
          </div>
        </div>
      ...
      {mode === "upload" && (
        <button onClick={onBack} style={{ background: "transparent", color: "#475569", border: "none", fontSize: 13, cursor: "pointer", marginTop: 12, padding: 0 }}>← Back</button>
      )}
    </div>
  );
}
  ...
  return (
    <div>
      <div style={{ marginBottom: 24 }}>
        ...
        <p style={{ fontSize: 13, color: "rgba(255,255,255,0.35)" }}>Upload your investor list and send personalized pitches in minutes.</p>
      </div>
      ...
      {isAtLimit ? (
        <div style={{ background: "rgba(124,58,237,0.08)", border: "1px solid rgba(124,58,237,0.25)", borderRadius: 14, padding: 32, textAlign: "center" }}>
          ...
          <p style={{ fontSize: 14, color: "rgba(255,255,255,0.4)", marginBottom: 24, lineHeight: 1.6, maxWidth: 400, margin: "0 auto 24px" }}>
            {plan === "starter"
              ? "Upgrade to Pro and unlock 500 pitches/month, Claude AI, deep investor research, and a full fundraising CRM."
              : "Upgrade to keep sending. Starter gives you 100 pitches/month. Pro gives you 500 plus Claude AI and deep investor research."}
          </p>
        ...
        <div style={{ background: "#0f172a", borderRadius: 16, padding: 24, border: "1px solid #1e293b" }}>
          ...
          {step === "upload" && (
            <div>
              <h2 style={{ fontSize: 18, fontWeight: 700, marginBottom: 4, color: "#f1f5f9" }}>Upload your investor list</h2>
              <p style={{ color: "#64748b", marginBottom: 20, fontSize: 13 }}>CSV with columns: name, email, firm (optional)</p>
              <div onClick={() => document.getElementById("csv-input").click()} style={{ border: "2px dashed #334155", borderRadius: 12, padding: "40px 24px", textAlign: "center", cursor: "pointer", background: "#0a0f1e" }}>
                <div style={{ fontSize: 32, marginBottom: 10 }}>📂</div>
                <div style={{ fontWeight: 600, color: "#e2e8f0", marginBottom: 4, fontSize: 14 }}>Drop CSV here or click to browse</div>
                <input id="csv-input" type="file" accept=".csv" style={{ display: "none" }} onChange={(e) => {
                  ...
                  reader.onload = (ev) => {
                    try {
                      ...
                      if (parsed[0]?.name && parsed[0]?.email) { setInvestors(parsed); setStep("describe"); }
                    } catch {}
                  };
          ...
          {step === "describe" && <DescribeStep onNext={(s) => { setStartup(s); setStep("review"); }} onBack={() => setStep("upload")} plan={plan} />}
  ...
  return (
    <div>
      <div style={{ marginBottom: 28 }}>
        ...
        <p style={{ fontSize: 13, color: "rgba(255,255,255,0.35)" }}>Find investors that match your startup.</p>
      </div>
      <div style={{ background: "#0f0f0f", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 16, padding: "48px 32px", textAlign: "center" }}>
        <div style={{ fontSize: 40, marginBottom: 16 }}>🔍</div>
        <h3 style={{ fontSize: 18, fontWeight: 700, color: "#fff", marginBottom: 8 }}>Investor database coming soon</h3>
        <p style={{ fontSize: 14, color: "rgba(255,255,255,0.35)", maxWidth: 380, margin: "0 auto 24px", lineHeight: 1.6 }}>
          We're building a curated database of thousands of active investors with fit scoring, activity tracking, and deep AI research. For now, upload your list as CSV on the Campaign tab.
        </p>
        {plan === "free" && (
          <a href="/upgrade" style={{ display: "inline-block", background: "#7c3aed", color: "#fff", padding: "10px 24px", borderRadius: 8, fontWeight: 700, fontSize: 13, textDecoration: "none" }}>
...
function AccountTab({ user, plan, pitchCount, onSignOut }) {
  ...
  return (
    <div>
      <div style={{ marginBottom: 28 }}>
        <h1 style={{ fontSize: 22, fontWeight: 800, color: "#fff", letterSpacing: "-0.5px", marginBottom: 4 }}>Account</h1>
        <p style={{ fontSize: 13, color: "rgba(255,255,255,0.35)" }}>Manage your plan and account settings.</p>
      </div>
      <div style={{ background: "#0f0f0f", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 14, padding: 24, marginBottom: 16 }}>
        <div style={{ fontSize: 11, fontWeight: 700, color: "rgba(255,255,255,0.3)", textTransform: "uppercase", letterSpacing: "1px", marginBottom: 16 }}>Profile</div>
        <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
          <div style={{ width: 48, height: 48, borderRadius: "50%", background: "linear-gradient(135deg,#7c3aed,#4f46e5)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 18, fontWeight: 800, color: "#fff" }}>
            {user?.email?.[0]?.toUpperCase() || "F"}
          </div>
          <div>
            <div style={{ fontSize: 15, fontWeight: 700, color: "#fff", marginBottom: 2 }}>{user?.user_metadata?.full_name || "Founder"}</div>
            <div style={{ fontSize: 13, color: "rgba(255,255,255,0.35)" }}>{user?.email}</div>
          </div>
        </div>
      </div>
      <div style={{ background: "#0f0f0f", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 14, padding: 24, marginBottom: 16 }}>
        <div style={{ fontSize: 11, fontWeight: 700, color: "rgba(255,255,255,0.3)", textTransform: "uppercase", letterSpacing: "1px", marginBottom: 16 }}>Current Plan</div>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
          <div>
            <div style={{ fontSize: 18, fontWeight: 800, color: "#fff", marginBottom: 2, textTransform: "capitalize" }}>{plan}</div>
            <div style={{ fontSize: 13, color: "rgba(255,255,255,0.35)" }}>{pitchCount} of {limit} pitches used</div>
          </div>
          <div style={{ fontSize: 11, fontWeight: 700, padding: "4px 12px", borderRadius: 99, background: plan === "pro" ? "rgba(74,222,128,0.1)" : plan === "starter" ? "rgba(124,58,237,0.15)" : "rgba(255,255,255,0.06)", color: plan === "pro" ? "#4ade80" : plan === "starter" ? "#a78bfa" : "rgba(255,255,255,0.3)", textTransform: "uppercase" }}>
            {plan}
          </div>
        </div>
        <div style={{ background: "rgba(255,255,255,0.04)", borderRadius: 99, height: 6, overflow: "hidden" }}>
          <div style={{ background: pitchCount / limit >= 0.9 ? "#f87171" : "linear-gradient(90deg,#7c3aed,#a78bfa)", height: "100%", borderRadius: 99, width: Math.min((pitchCount / limit) * 100, 100) + "%" }} />
        </div>
      </div>
      {plan !== "pro" && (
        <div style={{ background: "#0f0f0f", border: "1px solid rgba(124,58,237,0.2)", borderRadius: 14, padding: 24, marginBottom: 16 }}>
          <div style={{ fontSize: 11, fontWeight: 700, color: "rgba(255,255,255,0.3)", textTransform: "uppercase", letterSpacing: "1px", marginBottom: 16 }}>Upgrade</div>
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            {plan === "free" && (
              <button onClick={() => handleCheckout("starter")} disabled={checkoutLoading === "starter"} style={{ width: "100%", padding: "13px", borderRadius: 10, fontSize: 14, fontWeight: 700, cursor: "pointer", background: "rgba(124,58,237,0.15)", color: "#a78bfa", border: "1px solid rgba(124,58,237,0.25)" }}>
                {checkoutLoading === "starter" ? "Loading..." : "Starter — $29/mo · 100 pitches/month →"}
              </button>
            )}
            <button onClick={() => handleCheckout("pro")} disabled={checkoutLoading === "pro"} style={{ width: "100%", padding: "13px", borderRadius: 10, fontSize: 14, fontWeight: 700, cursor: "pointer", background: "#7c3aed", color: "#fff", border: "none" }}>
              {checkoutLoading === "pro" ? "Loading..." : "Pro — $79/mo · 500 pitches + Claude AI + CRM →"}
            </button>
          </div>
        </div>
      )}
      <div style={{ background: "#0f0f0f", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 14, padding: 24 }}>
        <div style={{ fontSize: 11, fontWeight: 700, color: "rgba(255,255,255,0.3)", textTransform: "uppercase", letterSpacing: "1px", marginBottom: 16 }}>Danger Zone</div>
        <button onClick={onSignOut} style={{ background: "transparent", color: "#f87171", border: "1px solid rgba(248,113,113,0.2)", borderRadius: 8, padding: "10px 20px", fontWeight: 600, fontSize: 13, cursor: "pointer" }}>Sign out</button>
      </div>
    </div>
  );
}
